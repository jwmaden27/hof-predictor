...
import { useNHLPlayerSearch } from '@/hooks/useNHLPlayerSearch.ts'
import { NHLSearchResults } from './NHLSearchResults.tsx'

interface NHLSearchBarProps {
  variant?: 'default' | 'large'
  className?: string
}

export function NHLSearchBar({ variant = 'default', className = '' }: NHLSearchBarProps) {
  const navigate = useNavigate()
  const { query, setQuery, results, isLoading, clearSearch } = useNHLPlayerSearch()
    ...
    <div className={`relative ${className}`}>
      <div className="relative">
        ...
        {query && (
          <button
            onClick={clearSearch}
            className={`absolute right-3 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 ${variant === 'large' ? 'top-4.5' : 'top-2.5'}`}
          >
