...
interface NHLMilestoneIndicatorProps {
  careerStats: Record<string, unknown> | null
  playerType: 'skater' | 'goalie'
}

/**
 * For rate stats (GAA, SV%), progress is mapped from rateFloor→threshold onto 0–100%.
...
function calculateProgress(
  ...
  if (isRate && rateFloor !== undefined) {
    if (lowerIsBetter) {
      // For GAA: lower is better, so rateFloor is the "good" end
      // Progress goes from threshold (bad) toward rateFloor (good)
      const range = threshold - rateFloor
      if (range <= 0) return 100
      return Math.min(Math.max(((threshold - value) / range) * 100, 0), 100)
    }
    const range = threshold - rateFloor
    if (range <= 0) return 100
...
export function NHLMilestoneIndicator({
  careerStats,
  playerType,
}: NHLMilestoneIndicatorProps) {
      ...
      const value =
        typeof rawValue === 'number'
          ...
          : typeof rawValue === 'string'
            ? parseFloat(rawValue)
            : 0
      const progress = calculateProgress(value, milestone.threshold, milestone.isRate, milestone.rateFloor, milestone.lowerIsBetter)
      const hofAveragePercent = calculateProgress(
        milestone.hofAverage, milestone.threshold, milestone.isRate, milestone.rateFloor, milestone.lowerIsBetter,
      )

      let reached: boolean
      if (milestone.lowerIsBetter) {
        reached = value > 0 && value <= milestone.threshold
      } else {
        reached = value >= milestone.threshold
      }
